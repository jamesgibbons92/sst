...
function stripImportsAndExports(source: string): string {
  return source.replace(/^import\s+.*$/gm, "").replace(/^export\s+.*$/gm, "");
}
...
export function cleanMarkdown(source: string): string {
  return source
    ...
    .map((part, i) =>
      ...
      i % 2 === 1 ? part : stripImportsAndExports(part)
    )
    .join("")
    ...
    .replace(/([^\n])\n(#{1,6} )/g, "$1\n\n$2")
    .replace(/\n(#{1,6} .+)\n([^\n])/g, "\n$1\n\n$2")
    // Replace <Changelog /> component
    .replace(/<Changelog\s*\/>/g, renderChangelog())
