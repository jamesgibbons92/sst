...
SST components are already updated for v7 — **no code changes are needed** if you are only using SST components without transforms or direct `@pulumi/aws` usage.
...
If you are using the AWS provider directly or using transforms, refer to the [Pulumi AWS v7 migration guide](https://www.pulumi.com/registry/packages/aws/how-to-guides/7-0-migration) for the full list of breaking changes.
